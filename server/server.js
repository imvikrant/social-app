...
passport_1.default.use(
  new passport_local_1.default.Strategy(
...
app.get('/profileImage/:id', (req, res) =>
  __awaiter(this, void 0, void 0, function*() {
    const id = req.params.id;
    console.log(id);
    let user;
    if (id === 'me') user = yield User.findById(req.user.id);
    else user = yield User.findById(id);
    if (user && user.profileImage) {
      res.send(user.profileImage.file);
    } else res.send('no image');
  })
);
app.get('/coverImage/:id', (req, res) =>
  __awaiter(this, void 0, void 0, function*() {
    const id = req.params.id;
    console.log(id);
    let user;
...
app.use(
  '/graphql',
  apollo_upload_server_1.apolloUploadExpress({
